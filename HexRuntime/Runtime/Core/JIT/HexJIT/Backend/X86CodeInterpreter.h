...
namespace RTJ::Hex::X86
{
	...
	*/
	struct NativePlatformInstruction
	{
		...
		INS_1(ADD_MI_I1, MI_F, 0x80);
		INS_1(ADD_MI_IU, MI_F, 0x81);
	...
	struct Operand
	{
		...
		static Operand FromImmediate(Int64 value, UInt8 coreType)
		{
	...
	private:
		void Emit(Instruction instruction, Operand const& left, Operand const& right, EmitPage* page = nullptr);
		void Emit(Instruction instruction, Operand const& operand, EmitPage* page = nullptr);
		void Emit(Instruction instruction, EmitPage* page = nullptr);
		...
		Operand UseImmediate(ConstantNode* constant, bool forceRegister = false);
		...
		void CodeGenForReturn(BasicBlock* basicBlock);

		void CodeGenFor(BinaryArithmeticNode* binaryNode);
